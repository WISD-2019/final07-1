...
class SocialController extends Controller
{
    ...
    public function getSocialCallback($provider,Request $request)
    {
        ...
        if(!empty($user)){
            ...
            //建立帳號
            $new_user = new User(['email' => $socialite_user->email,'name' => $socialite_user->name]);
            $new_user->provider=$provider;
            $new_user->save();
            $new_socialUser = new SocialUser (['user_id' => $new_user->id,'provider_user_id' => $socialite_user->id,'provider'=> $provider]);
            $new_socialUser->save();
            $login_user=$new_user;
        }
